...
export async function PUT(
  ...
  try {
    ...
    if (status === 'WAITING' || status === 'ENGAGED' || status === 'IN_PROGRESS' || status === 'COMPLETED' || status === 'NO_SHOW') {
      ...
      await logActivity({
        ...
        type: types[status] as 'PATIENT_CHECKIN' | 'APPOINTMENT_COMPLETED' | 'APPOINTMENT_NO_SHOW',
