  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4">
      <div className="flex-1 min-w-0 overflow-hidden rounded-md border border-gray-200 bg-white">
        <div className="grid gap-2 border-b border-gray-200 p-2 md:grid-cols-2">
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[800px]">
            <div className="grid grid-cols-[2fr_1fr_1fr_.55fr_.55fr_.55fr_1fr_40px] gap-1.5 border-b border-gray-200 bg-gray-50 px-2 py-1.5 text-[10px] font-semibold text-gray-500">
              <span>DRUG NAME</span>
              <span>STRENGTH</span>
              <span>DURATION</span>
              <span>M</span>
            ...
            {medicines.map((medicine, idx) => (
              <div key={idx} className="grid grid-cols-[2fr_1fr_1fr_.55fr_.55fr_.55fr_1fr_40px] gap-1.5 border-b border-gray-100 px-2 py-2">
                <div>
                  <DrugNameInput
                    clinicId={clinicId}
                  ...
                  {medicine.instruction === null ? (
                    <button onClick={() => updateMedicine(idx, { instruction: '' })} className="mt-1 text-[10px] text-blue-600 hover:underline">+ add instruction</button>
                  ) : (
                ...
                <div className="flex gap-1 text-[10px]">
                  {foodOptions.map((option) => (
                    <label key={option} className="flex items-center gap-0.5"><input type="radio" checked={medicine.beforeAfterFood === option} onChange={() => updateMedicine(idx, { beforeAfterFood: option })} /> {option}</label>
                  ))}
                </div>
                <button onClick={() => setMedicines((prev) => (prev.length > 1 ? prev.filter((_, i) => i !== idx) : prev))} className="rounded p-1 text-gray-400 hover:bg-red-50 hover:text-red-600"><X className="h-3 w-3" /></button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="border-b border-gray-100 p-2">
          <button onClick={() => setMedicines((prev) => [...prev, emptyMedicine()])} className="inline-flex items-center gap-1 text-xs text-blue-600 hover:underline"><Plus className="h-3 w-3" /> Add Drug Row</button>
        </div>
        ...
        {!notesExpanded && (
          <div className="border-b border-gray-100 p-2">
            <button onClick={() => setNotesExpanded(true)} className="text-xs text-blue-600 hover:underline">+ Add notes</button>
          </div>
        )}
        ...
        <div className="flex justify-end gap-2 p-2">
          <button onClick={onCancel} className="rounded-md border border-gray-300 px-3 py-1.5 text-xs hover:bg-gray-50">Cancel</button>
          <button onClick={() => setTemplateOpen(true)} disabled={!canManageTemplates} className="inline-flex items-center gap-1 rounded-md border border-blue-200 px-3 py-1.5 text-xs text-blue-700 hover:bg-blue-50 disabled:opacity-40"><Save className="h-3 w-3" /> Save as Template</button>
          <button onClick={savePrescription} disabled={saving} className="rounded-md bg-blue-600 px-3 py-1.5 text-xs text-white hover:bg-blue-700 disabled:opacity-50">{saving ? 'Saving...' : 'Save Prescription'}</button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4">
          <div className="w-full max-w-sm rounded-md bg-white p-4 shadow-xl">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={() => setTemplateOpen(false)} className="rounded p-1 text-gray-400 hover:bg-gray-100"><X className="h-4 w-4" /></button>
            </div>
            ...
            <button onClick={saveTemplate} className="mt-3 w-full rounded-md bg-blue-600 px-3 py-2 text-sm text-white hover:bg-blue-700">Save</button>
          </div>
        </div>
      )}
    </div>
